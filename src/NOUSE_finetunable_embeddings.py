...
# ==============================================================================
""" Embedding layer that can be fine-tuned """
...
class Finetunable_Embedding(Layer):
  ...
  def __init__(self,
    ...
    '''
    super(Finetunable_Embedding, self).__init__(dtype=dtype, **kwargs)
  ...
  def build(self, input_shape):
    ...
    if context.executing_eagerly() and context.context().num_gpus():
      with ops.device('cpu:0'):
        ...
        if self.additional_input_dim is not None:
            self.additional_embeddings = self.add_weight(
                shape=(self.additional_input_dim, self.output_dim),
                initializer=self.embeddings_initializer,
                name='addtional_embeddings',
                regularizer=self.embeddings_regularizer,
                constraint=self.embeddings_constraint)    
    else:
      self.embeddings = self.add_weight(
  ...
  def call(self, inputs):
    ...
    np_ts = lambda nparray: tensorflow.convert_to_tensor(nparray, dtype=dtype)
    ...
    add_out = embedding_ops.embedding_lookup(self.additional_embeddings, add_inputs)
    
    result_numpy = np.zeros(shape=(np_inputs.shape[0], self.output_dim), dtype=np.float32)
    out = tf.zeros(shape=(inputs.shape[0], self.output_dim), dtype=tf.dtypes.float32)
    return pre_out
  ...
  def get_config(self):
    ...
    base_config = super(Finetunable_Embedding, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))

if __name__ == 'how to use it':
    input_dim, additional_input_dim, output_dim = 3,2,5
    self = Finetunable_Embedding(input_dim, output_dim, additional_input_dim)
 
    import numpy as np   
    import tensorflow as tf
    inputs = np.random.choice(range(input_dim), size=20)
